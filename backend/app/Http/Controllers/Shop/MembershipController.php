 ...
 *
 * The card is never handled here. The client sends a Stripe payment method
 * token, Stripe stores the card against a customer, and every later charge
 * uses that saved reference. Each attempt, successful or not, is written to
 * the payment_transactions ledger.
 */
class MembershipController extends Controller
{
    /** Monthly membership price, in the configured currency. */
    ...
    /**
     * Current membership state plus the caller's own ledger entries.
     */
    ...
    /**
     * Start a membership: save the tokenised card, take the first payment, and
    ...
    public function subscribe(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                // Stripe test fixtures such as pm_card_visa, or a real token
                // produced by Stripe.js in a production build.
    ...
    /**
     * Charge the saved card for another period.
     *
     * Exposed as an endpoint so a renewal can be demonstrated on demand. In
     * production the same method would be called by a scheduled command once
     * the current period lapses.
     */
    ...
    /**
     * Stop auto renewal. Access already paid for is honoured until it lapses.
     */
    ...
    /**
     * The accounting ledger. Customers see their own rows, admins see everything.
     */
    public function ledger(Request $request): JsonResponse
    {
        try {
            ...
            // Running totals for the summary strip, over the same visible scope.
    ...
    /**
     * Run one charge, write the ledger row either way, and extend access only
    ...
    private function charge(User $user, string $type, string $description): JsonResponse
    {
        ...
        // Extend from whichever is later: now, or the unused remainder of the
        // current period, so renewing early never costs the customer days.
