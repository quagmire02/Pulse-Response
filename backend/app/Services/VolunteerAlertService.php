...
/**
 * Scans the immediate area around an incident and pulls in nearby volunteers
 * who can begin first aid before the ambulance arrives.
 *
 * The radius is deliberately small. Alerting a whole district would produce
 * noise nobody acts on; a couple of kilometres is roughly the distance someone
 * can cover on foot or by bike inside an ambulance response window.
 */
class VolunteerAlertService
{
    /** How far around the patient to look, in kilometres. */
    public const RADIUS_KM = 2.5;

    /** Never wake more than this many people for one incident. */
    ...
    /**
     * Notify every on duty volunteer inside the radius.
     *
     * @return array{notified: int, radius_km: float}
     */
    public function notifyNearby(EmergencyAlert $alert, ?string $etaNote = null): array
    {
        ...
        try {
            ...
            foreach ($candidates as $volunteer) {
                // The patient should not be alerted about their own emergency.
            ...
            // Closest first, so the people most likely to arrive get the call.
            ...
            foreach ($inRange as $entry) {
                ...
                // firstOrCreate keeps a re-scan from double notifying.
            ...
            // A failure here must never take down the ambulance dispatch itself.
