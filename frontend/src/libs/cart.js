...
/**
 * The cart endpoint replaces the whole basket in one call, so every helper here
 * reads the current lines first and posts the full list back.
 */
...
/**
 * Two lines are the same basket entry when they point at the same product in the
 * same mode. Rentals additionally have to cover the same dates, otherwise a
 * second booking would silently overwrite the first.
 */
...
/**
 * Add one line to the cart, merging quantities when the same entry is already
 * present. Returns { success } or { error }.
 */
...
/**
 * Replace the quantity of one existing line, addressed by its cart item id.
 */
...
/**
 * Drop one line from the cart, addressed by its cart item id.
 */
