...
export default function DriverPage() {
  ...
  // Poll for a new assignment while on duty, so a dispatched call shows up
  // without the driver refreshing.
  useEffect(() => {
    if (!onDuty) return
  ...
  /**
   * The browser reports this device's own GPS. There is no way to read a
   * location from a phone number, so the driver device has to push it.
   */
  const startTracking = () => {
    ...
    // Send on a fixed timer rather than on every GPS event, which would flood
    // the API while the vehicle is moving.
