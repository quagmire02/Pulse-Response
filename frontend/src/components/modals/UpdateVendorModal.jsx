...
import { updateVendorAction } from "@/actions/vendorActions"
import styles from "./UpdateVendorModal.module.css"

export default function UpdateVendorModal({ vendor, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      // Send fields only if they have values or are updated
      if (value) {
    ...
    try {
      const result = await updateVendorAction(vendor.user_id, submissionData)
      if (result.error) {
        setErrors(result.error)
      } else {
        setSuccessMsg(result.success || "Vendor updated successfully!")
        setTimeout(() => {
          onSuccess()
        }, 1500)
      }
    } catch (err) {
      setErrors({ error: "Failed to update vendor." })
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <div className={styles.header}>
          <h2 className={styles.title}>Update Vendor Profile</h2>
